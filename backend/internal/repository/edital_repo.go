...
// DeleteMockAnalyses removes analyses generated by the benchmark-only edital parser.
func (r *EditalRepository) DeleteMockAnalyses(ctx context.Context) (int64, error) {
	tx, err := r.db.BeginTx(ctx, nil)
	if err != nil {
		return 0, fmt.Errorf("failed to begin mock analyses cleanup tx: %w", err)
	}
	defer func() {
		_ = tx.Rollback()
	}()

	where := `(orgao = ? AND numero_edital LIKE ?) OR ((orgao = ? OR orgao = ?) AND numero_edital LIKE ?)`
	args := []interface{}{
		"Prefeitura Municipal de Parambu", "Concorrência Eletrônica nº 2026.08.04.001%",
		"Prefeitura Municipal de Baixio", "Prefeitura Municipal de Baixio (Secretaria de Educação)", "Concorrência Eletrônica nº 30.07.01.2026CE%",
	}
	analysisIDs := "SELECT id FROM edital_analysis WHERE " + where

	for _, table := range []string{
		"edital_pegadinha",
		"edital_qualificacao_tecnica",
		"edital_checklist_item",
		"edital_indice_financeiro",
		"edital_requisito_habilitacao",
	} {
		stmt := fmt.Sprintf("DELETE FROM %s WHERE analysis_id IN (%s)", table, analysisIDs)
		if _, err := tx.ExecContext(ctx, stmt, args...); err != nil {
			return 0, fmt.Errorf("failed to delete mock edital child rows from %s: %w", table, err)
		}
	}

	result, err := tx.ExecContext(ctx, "DELETE FROM edital_analysis WHERE "+where, args...)
	if err != nil {
		return 0, fmt.Errorf("failed to delete mock edital analyses: %w", err)
	}
	deleted, err := result.RowsAffected()
	if err != nil {
		return 0, fmt.Errorf("failed to count deleted mock edital analyses: %w", err)
	}

	if err := tx.Commit(); err != nil {
		return 0, fmt.Errorf("failed to commit mock analyses cleanup: %w", err)
	}
	return deleted, nil
}
